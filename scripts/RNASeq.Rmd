...
Copy annotated DEGs to clipboard

```{r}
gensas.blast %>% filter(e_value<1e-10) %>%
filter(id %in% 
rownames(diffex.treatment)) %>% write_clip()

diffex.treatment %>% as.data.frame() %>%
  rownames_to_column("geneid") %>% 
  left_join(gensas.blast %>% filter(e_value<1e-10), 
            by=c("geneid"="id")) %>%
  arrange(log2FoldChange) %>%
  mutate(across(c(2:5), round, 2)) %>%
  mutate(across(c(6:7), round, 4)) %>% 
  write_csv(file = "../results/DESeq2/DEGs.csv")
```
