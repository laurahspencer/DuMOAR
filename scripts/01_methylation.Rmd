...
The following samples have low CpG methylation: 5, 6, 9, 13, 14, 16, 19. Check out each sample individually 
...
meth_unite_nolows <- reorganize(methylObj = meth_unite, 
                                sample.ids = c("1", "2", "3", "4", "7", "8", "10", "11", "12", "15", "17", "18", "20"),
                                treatment = as.numeric((sample.info %>% 
                                                          arrange(sample_seq))$high_or_low_co2)[c(1:4, 7, 8, 10, 11, 12, 15, 17, 18, 20)])
PCASamples(meth_unite_nolows)
```

```{r}
(sample.info %>% arrange(sample_seq))[c(1:4, 7, 8, 10, 11, 12, 15, 17, 18, 20),]
```
## ERROR FOUND IN THE BELOW CODE CHUNK 

THE UNITE() FUNCTION ERRONEOUSLY RE-ASSIGNS SAMPLE IDS USING THE NUMBERS 1-13, WHILE THE CORRECT SAMPLE ID'S ARE 1,2,3,4,7,8,10,11,12,15,17,18,20.  

```{r}
meth_obj_nolow <- reorganize(methylObj = meth_obj, sample.ids = c("1", "2", "3", "4", "7", "8", "10", "11", "12", "15", "17", "18", "20"),
                             treatment = as.numeric((sample.info %>% 
                                                          arrange(sample_seq))$high_or_low_co2)[c(1:4, 7, 8, 10, 11, 12, 15, 17, 18, 20)])
...
```{r}
meth_10x[[1]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[2]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[3]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[4]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[5]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[6]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[7]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[8]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[9]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[10]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[11]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[12]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
meth_10x[[13]] %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
```

```{r}
meth_unite_10x %>% getData() %>% filter(chr=="PGA_scaffold0__40_contigs__length_4818635" & start==527803)
```
...
plotCustomization <- (sample.info %>% arrange(sample_seq))[c(1:4, 7, 8, 10, 11, 12, 15, 17, 18, 20),c("high_or_low_co2", "sample_seq", "tank")] %>%
...
```{r}
meth_unite_5x #352,376 loci
meth_unite_10x #130,032 loci 
```
...
mean(perc.meth[,1:5], na.rm=TRUE) # 89.6% <-- Low CO2 exposed - 79.5%
mean(perc.meth[,6:12], na.rm=TRUE) # 89.6%  <-- High CO2 exposed - 78.9% 
...
paste("No. of loci in filtered dataset:", nrow(perc.meth)) #130,032 loci 
paste("No. of samples:", ncol(perc.meth)) #13 sampes 
```
...
Looks as though there is slightly higher methylation rates in the High pCO2 treatment (red distribution is shifted to the right a bit)
...
Find location of peaks in each  treatment. Used this as a reference: http://ianmadd.github.io/pages/PeakDensityDistribution.html

```{r}
## Find largest peak 

# Find largest peak for High pCO2
density(subset(meth_unite_10x_reshaped,  treatment=="H" & percMeth!="NA")$percMeth) # Y maximum is 4.895e-02

# Find where that maximum lies (this gives the index #)
which.max(density(subset(meth_unite_10x_reshaped,  treatment=="H" & percMeth!="NA")$percMeth)$y) #answer = index #496

# Find the x value where the largest Y value is
density(subset(meth_unite_10x_reshaped,  treatment=="H" & percMeth!="NA")$percMeth)$x[496] #H max (4.895e-02) is @ 99.78%

##--------------

# Find largest peak for Low pCO2
density(subset(meth_unite_10x_reshaped,  treatment=="L" & percMeth!="NA")$percMeth) # Y maximum is 0.0469560  

which.max(density(subset(meth_unite_10x_reshaped,  treatment=="L" & percMeth!="NA")$percMeth)$y) #answer = 498

density(subset(meth_unite_10x_reshaped,  treatment=="L" & percMeth!="NA")$percMeth)$x[498] #L max (0.0469560  ) is 99.94%
...
density(subset(meth_unite_10x_reshaped,  treatment=="H" & percMeth!="NA" & percMeth<98)$percMeth) # Y maximum is 5.281e-02

# Find where that maximum lies (this gives the index #)
which.max(density(subset(meth_unite_10x_reshaped,  treatment=="H" & percMeth!="NA" & percMeth<98)$percMeth)$y) #answer = index #478

# Find the x value where the largest Y value is
density(subset(meth_unite_10x_reshaped,  treatment=="H" & percMeth!="NA" & percMeth<98)$percMeth)$x[478] #H max (5.281e-02) is @ 94.53%

#---
# Find 2nd largest peak for Low pCO2
density(subset(meth_unite_10x_reshaped,  treatment=="L" & percMeth!="NA" & percMeth<98)$percMeth) # Y maximum is 4.953e-02

# Find where that maximum lies (this gives the index #)
which.max(density(subset(meth_unite_10x_reshaped,  treatment=="L" & percMeth!="NA" & percMeth<98)$percMeth)$y) #answer = index #478

# Find the x value where the largest Y value is
density(subset(meth_unite_10x_reshaped,  treatment=="L" & percMeth!="NA" & percMeth<98)$percMeth)$x[478] #L max (4.953e-02) is @ 94.25%

# Very similar 2nd peak CpG methylation - High pCO2 = 94.53%, and Low pCO2 = 94.25% (only slightly higher in high pCO2 treatment)

#----- Double check # of loci 
meth_unite_10x_reshaped %>%
  filter( treatment=="H" & percMeth!="NA") %>%
  dplyr::select(chr, start) %>% unique() %>% nrow() #130,032

meth_unite_10x_reshaped %>%
  filter( treatment=="L" & percMeth!="NA") %>%
  dplyr::select(chr, start) %>% unique() %>% nrow() #130,032 (good they are the same)

#----- average % meth by pop

# High pCO2 mean = 79.94%, median = 89.09%
meth_unite_10x_reshaped %>%
  filter( treatment=="H" & percMeth!="NA") %>%
  dplyr::select(percMeth) %>% summary()

# Low (control) pCO2 mean = 78.64%, median = 88.10%
meth_unite_10x_reshaped %>%
  filter( treatment=="L" & percMeth!="NA") %>%
  dplyr::select(percMeth) %>% summary()
```

Takeaway = high and low (control) pCO2 have very similar %CpG distributions, but slightly higher methylation rates in the high pCO2 reared crab. 
...
```
DMLs on average have lower % methylation in the high pCO2 treatment 
...
sample.info[c("sample_seq", "tank")]
meth_unite_10x_reshaped %>% dplyr::select(sample, treatment) %>% distinct()

#DMLs_heatmap <- 
  meth_unite_10x_reshaped %>% dplyr::select(sample, treatment) %>% distinct() %>% left_join(sample.info[c("sample_seq", "tank")] %>% mutate(sample_seq=as.character(sample_seq)), by=c("sample"="sample_seq")) %>% arrange(treatment, tank)
