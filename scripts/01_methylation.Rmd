...
meth_unite_nolows <- reorganize(methylObj = meth_unite, 
                                sample.ids = c("1", "2", "3", "4", "7", "8", "10", "11", "15", "17", "18", "20"),
                                treatment = as.numeric((sample.info %>% 
                                                          arrange(sample_seq))$high_or_low_co2)[c(1:4, 7, 8, 10, 11, 15, 17, 18, 20)])
PCASamples(meth_unite_nolows)
```

```{r}
(sample.info %>% arrange(sample_seq))[c(1:4, 7, 8, 10, 11, 15, 17, 18, 20),]
```

```{r}
meth_obj_nolow <- reorganize(methylObj = meth_obj, sample.ids = c("1", "2", "3", "4", "7", "8", "10", "11", "15", "17", "18", "20"),
                             treatment = as.numeric((sample.info %>% 
                                                          arrange(sample_seq))$high_or_low_co2)[c(1:4, 7, 8, 10, 11, 15, 17, 18, 20)])
...
meth_unite_10x=methylKit::unite(meth_10x, destrand=TRUE, min.per.group=5L)
...
plotCustomization <- (sample.info %>% arrange(sample_seq))[c(1:4, 7, 8, 10, 11, 15, 17, 18, 20),c("high_or_low_co2", "sample_seq", "tank")] %>%
...
points(PCA.figure, "sites", col = as.character(plotCustomization$color), pch = plotCustomization$symbol, cex = 2.5)  #use points
...
mtext(side = 1, text = "PC 1 (13.7%)", line = 3, cex = 1.5) #Add x-axis label
axis(side =  2, labels = TRUE, col = "grey80", cex.axis = 1.7) #Add y-axis
mtext(side = 2, text = "PC 2 (10.5%)", line = 3, cex = 1.5) #Add y-axis label
legend("bottomright", 
       pch = c(16, 17), 
...
NOTE: this PCA was run with _all_ methylated loci data after filtering for 105x across at minimum 5 samples per treatment. 
...
# Also try to filter for 10x across all samples? 
...
#text(PCA.figure_allsamples$sites, col = as.character(plotCustomization_allsamples$color), labels=plotCustomization_allsamples$tank, cex=1.5) #use tank number 
